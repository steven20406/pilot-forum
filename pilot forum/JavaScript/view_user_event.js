/**
 * Created by Steven 臧 on 2017/8/16.
 */
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if(user.twitter_username=='None'){$('#userdata').append(
                "<tr>" +
            ...
            if (resp.content.length == 0) {
                ...
                var count = resp.content.length;
                var content = resp.content;
                for (var index in content) {
                    var number = count - index - 1;

                    $('#contentarea').append("<tr>" +
                        "<td><a href='specific_content.html?id=" + content[number]._id + "'>" + content[number].subject + "</a></td>" +
                        "<td class='tabledata'>" + content[number].comment.length + "</td>" +
                        "<td class='tabledata'>" + content[number].yeardate + "</td>" +
                        "</tr>"
                    );
                }
            }
